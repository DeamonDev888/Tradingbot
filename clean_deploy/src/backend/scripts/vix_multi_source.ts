...
if (foundFiles.length > 0 && vixData) {
  ...
  console.log(
    `   Variation: ${vixData.change >= 0 ? '+' : ''}${vixData.change.toFixed(2)}$ (${vixData.changePercent >= 0 ? '+' : ''}${vixData.changePercent.toFixed(2)}%)`
  );
  ...
  FALLBACK_VIX_DATA.forEach((data, index) => {
    ...
    console.log(
      `   Variation: ${data.change >= 0 ? '+' : ''}${data.change}$ (${data.changePercent >= 0 ? '+' : ''}${data.changePercent}%)`
    );
  });

  // Calculer une moyenne des sources
  const avgPrice =
    FALLBACK_VIX_DATA.reduce((sum, d) => sum + d.price, 0) / FALLBACK_VIX_DATA.length;
  const avgChange =
    FALLBACK_VIX_DATA.reduce((sum, d) => sum + d.change, 0) / FALLBACK_VIX_DATA.length;
  const avgChangePercent =
    FALLBACK_VIX_DATA.reduce((sum, d) => sum + d.changePercent, 0) / FALLBACK_VIX_DATA.length;
  ...
  console.log(
    `   Variation moyenne: ${vixData.change >= 0 ? '+' : ''}${vixData.change.toFixed(2)}$ (${vixData.changePercent >= 0 ? '+' : ''}${vixData.changePercent.toFixed(2)}%)`
  );
  console.log(`   Source: ${vixData.source}`);
