...
export class RougePulseAgent extends BaseAgentSimple {
  ...
  private classifyEventsByImpact(events: any[]): {
    critical: any[]; // Rouge + gras = change vraiment le marché
    high: any[]; // Rouge = impact fort
    medium: any[]; // Jaune/Orange = impact moyen
    low: any[]; // Normal = faible impact
  } {
    const now = new Date();
  ...
  private groupEventsByImportance(classified: any): any {
    ...
    levels.forEach(level => {
      classified[level].forEach((event: any) => {
