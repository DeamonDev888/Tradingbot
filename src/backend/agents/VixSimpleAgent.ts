...
export class VixSimpleAgent {
  ...
  async analyzeVixStructure(): Promise<VixAnalysisData | { error: string }> {
  ...
  private async getVixDataFromDatabase(): Promise<VixData[]> {
    try {
  ...
  private async getVvixDataFromDatabase(): Promise<VixData[]> {
    try {
      const result = await this.pool.query(`
        SELECT * FROM vix_data
  ...
  private determineVolatilityRegime(vixValue: number): string {
    if (vixValue >= 35) return 'CRISIS';
    if (vixValue >= 25) return 'ELEVATED';
    if (vixValue >= 18) return 'NORMAL';
    if (vixValue >= 12) return 'CALM';
    return 'EXTREME_CALM';
  }

  private determineVixTrend(vixData: VixData[]): string {
    if (vixData.length < 5) return 'NEUTRAL';
  ...
  private calculateSpread(vixData: VixData[]): number {
    const values = vixData.slice(0, 10).map(d => d.value);
    return Math.max(...values) - Math.min(...values);
  }

  private generateExpertSummary(consensusValue: number, regime: string, trend: string): string {
  ...
  private determineTradingStrategy(regime: string, trend: string): string {
    if (regime === 'CRISIS') return 'Defensive positioning, volatility selling';
    if (regime === 'ELEVATED') return 'Selective opportunities, hedge strategies';
    if (trend === 'BULLISH') return 'Momentum strategies, breakout plays';
    if (trend === 'BEARISH') return 'Short positioning, protection strategies';
    return 'Range-bound strategies, mean reversion';
  }

  private determineTimeHorizon(regime: string): string {
    if (regime === 'CRISIS' || regime === 'ELEVATED') return 'Short-term (1-5 days)';
    if (regime === 'NORMAL') return 'Medium-term (1-4 weeks)';
  ...
  private determineRiskManagement(vixValue: number): string {
    if (vixValue >= 30) return 'Maximum protection, diversified hedging';
    if (vixValue >= 20) return 'Enhanced protection, selective hedging';
    if (vixValue >= 15) return 'Standard protection, minimal hedging';
    return 'Basic protection, limited hedging';
  }

  private calculateTargetLevels(vixValue: number, regime: string): number[] {
    const levels = [];
  ...
  private identifyCatalysts(vixData: VixData[]): string[] {
    const catalysts = [];
  ...
  private categorizeVixLevel(vixValue: number): string {
    if (vixValue >= 35) return 'Extreme Fear';
    if (vixValue >= 25) return 'High Fear';
    if (vixValue >= 18) return 'Moderate Fear';
    if (vixValue >= 12) return 'Low Fear';
    return 'Extreme Greed';
  }

  private generateInterpretation(consensusValue: number, regime: string): string {
  ...
  private calculateExpectedVolatility(vixValue: number, period: string): number {
  ...
  private determineSignalStrength(vixData: VixData[]): string {
    if (vixData.length < 10) return 'Low';
  ...
  private calculateVolatility(data: VixData[]): number {
  ...
  private generateActionableRecommendations(expertAnalysis: any): string[] {
  ...
  private async saveAnalysisToDatabase(analysis: VixAnalysisData): Promise<void> {
    try {
      const query = `
  ...
  private async saveAnalysisToMarkdown(analysis: VixAnalysisData): Promise<void> {
    try {
      ...
      if (analysis.current_vix_data.sources.length > 0) {
        ...
        analysis.current_vix_data.sources.forEach((source: VixData, i) => {
      ...
      if (analysis.expert_volatility_analysis) {
        ...
        if (expert.catalysts.length > 0) {
          markdown += `\n### Catalyseurs\n\n`;
          expert.catalysts.forEach((catalyst: string, i) => {
        ...
        if (expert.key_insights.length > 0) {
          markdown += `\n### Insights Clés\n\n`;
          expert.key_insights.forEach((insight: string, i) => {
