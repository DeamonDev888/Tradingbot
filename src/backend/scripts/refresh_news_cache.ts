 ...
 *
 * Script pour le rafraîchissement intelligent du cache de news.
 *
 * Fonctionnalités:
...
async function main() {
  ...
  console.log(
    `Options: Force=${options.force}, Cleanup=${options.cleanup}, Stats=${options.stats}, Hours=${options.hours || 'default'}`
  );
  console.log('');
  ...
  try {
    ...
    if (options.stats) {
      ...
      if (stats.error) {
        ...
        console.log(
          `Bullish: ${stats.news?.bullish || 0}, Bearish: ${stats.news?.bearish || 0}, Neutral: ${stats.news?.neutral || 0}`
        );
        ...
        if (stats.sources && stats.sources.length > 0) {
          console.log('\n📰 Source Status:');
          stats.sources.forEach((source: any) => {
            const status = source.is_active ? '✅' : '❌';
            const lastScraped = source.last_scraped_at
              ? new Date(source.last_scraped_at).toLocaleString()
              : 'Never';
            console.log(
              `  ${status} ${source.name}: Success ${source.success_count}, Errors ${source.error_count}, Last: ${lastScraped}`
            );
          });
        }
      }
    ...
    console.log(
      `🕐 Cache Status: ${isFresh ? '✅ FRESH' : '⚠️ STALE'} (${options.hours || 2}h threshold)`
    );
