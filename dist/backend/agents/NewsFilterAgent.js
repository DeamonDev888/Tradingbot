...
export class NewsFilterAgent extends BaseAgentSimple {
    ...
    async runFilterCycle() {
        ...
        try {
            ...
            for (let i = 0; i < pendingItems.length; i += batchSize) {
                ...
                console.log(`[${this.agentName}] Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(pendingItems.length / batchSize)} (${batch.length} items)`);
            ...
            console.log(`[${this.agentName}] ✅ Filter cycle completed: ${pendingItems.length} items processed`);
        }
        catch (error) {
    ...
    async scrapeAndSaveXNews() {
        try {
            ...
            if (result.success && result.items.length > 0) {
                ...
                const xNewsItems = result.items.map((xItem) => ({
                    title: xItem.title,
                    source: xItem.source,
                    url: xItem.url,
     ...
     */
    async saveXNewsToDatabase(xNewsItems) {
        if (!xNewsItems || xNewsItems.length === 0)
            return;
      ...
      `);
            ...
            console.log(`[${this.agentName}] 💾 Saved ${savedCount}/${xNewsItems.length} X news items to database`);
        }
        catch (error) {
    ...
    buildPrompt(batch) {
        const itemsJson = JSON.stringify(batch.map(b => ({
            id: b.id,
            title: b.title,
                ...
                try {
                    ...
                    if (event.type === 'say' && event.say === 'completion_result') {
                        ...
                        if (event.metadata && event.metadata.results && Array.isArray(event.metadata.results)) {
                            console.log(`✅ Stratégie 0a (Metadata) réussie: ${event.metadata.results.length} résultats`);
                            return event.metadata.results;
                        }
                        ...
                        if (event.content) {
                            try {
                                ...
                                if (contentJson.results && Array.isArray(contentJson.results)) {
                                    console.log(`✅ Stratégie 0b (Content JSON) réussie: ${contentJson.results.length} résultats`);
...
${rawOutput.substring(0, 1000)}
    ...
    async updateDatabase(results) {
        ...
        `, [res.relevance_score, res.category, res.summary, res.id]);
                console.log(`[${this.agentName}] Updated item ${res.id}: ${res.category} (${res.relevance_score}/10) -> processed (was ${res.processing_status})`);
            }
        }
        finally {
            client.release();
