...
exports.RougePulseAgentFixed = void 0;
const BaseAgentSimple_1 = require("./BaseAgentSimple");
const NewsDatabaseService_1 = require("../database/NewsDatabaseService");
const RougePulseDatabaseService_1 = require("../database/RougePulseDatabaseService");
const dotenv = __importStar(require("dotenv"));
dotenv.config();
class RougePulseAgentFixed extends BaseAgentSimple_1.BaseAgentSimple {
    dbService;
    rpDbService;
    constructor() {
        super('rouge-pulse-agent-fixed');
        this.dbService = new NewsDatabaseService_1.NewsDatabaseService();
        this.rpDbService = new RougePulseDatabaseService_1.RougePulseDatabaseService();
    }
}
exports.RougePulseAgentFixed = RougePulseAgentFixed;
/**
 * Analyse du calendrier économique avec scoring avancé
 */
async;
analyzeMarketSentiment(_forceRefresh, boolean = false);
...
classifyEventsByImpact(events, any[]);
{
    const now = new Date();
...
isKeyMarketIndicator(eventName, string);
boolean;
{
    const keyIndicators = [
        'fomc',
...
calculateForecastChange(forecast, string, previous, string);
string;
{
...
calculateSurprisePotential(e, any);
'HIGH' | 'MEDIUM' | 'LOW';
{
    const score = e.calculated_score || e.importance || 1;
    if (score >= 3.5)
...
identifyMarketMovers(criticalEvents, any[]);
any[];
{
...
generateCriticalAlerts(criticalEvents, any[]);
any[];
{
    const now = new Date();
    const next24h = new Date(now.getTime() + 24 * 60 * 60 * 1000);
...
generateAdvancedSummary(classified, any, criticalAlerts, any[]);
string;
{
...
calculateVolatilityScore(classified, any);
number;
{
    let score = 0;
...
groupEventsByImportance(classified, any);
any;
{
    const grouped = {};
...
getNext24HoursAlerts(classified, any);
any[];
{
    const now = new Date();
    const next24h = new Date(now.getTime() + 24 * 60 * 60 * 1000);
...
generateCalendarSummary(high, any[], medium, any[]);
string;
{
