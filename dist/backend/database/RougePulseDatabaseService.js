...
export class RougePulseDatabaseService {
    ...
    async getLatestAnalysis() {
        ...
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT * FROM rouge_pulse_analyses_v2
    ...
    async getRecentAnalyses(daysBack = 7) {
        if (!this.pool)
            return [];
        const client = await this.pool.connect();
        try {
...
//# sourceMappingURL=RougePulseDatabaseService.js.map