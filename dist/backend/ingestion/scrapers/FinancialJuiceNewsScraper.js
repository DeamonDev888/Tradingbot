            ...
            try {
                ...
                const newsPromises = topItems.map(async (item) => {
                    ...
                    if (title && link) {
                        ...
                        console.log(`[FinancialJuiceNewsScraper] ⚠️ Skipping article due to insufficient content: ${title}`);
                    }
                    return null;
                });
                ...
                if (axios.isAxiosError(error) && error.response?.status === 429) {
                    console.warn(`[FinancialJuice] Rate limited (429). Retrying in ${delay}ms... (${retries} retries left)`);
...
//# sourceMappingURL=FinancialJuiceNewsScraper.js.map