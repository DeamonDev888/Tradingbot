        ...
        for (const block of sqlBlocks) {
            ...
            if (sql.trim()) {
                ...
                try {
                    ...
                    console.error(`❌ ERREUR dans le bloc ${blockNumber}:`, error instanceof Error ? error.message : String(error));
                }
                console.log('\n' + '='.repeat(60));
            }
        }
...
async function analyzeScriptErrors(client) {
    ...
    `;
        ...
        console.error("❌ Erreur lors de l'analyse des erreurs de script:", error instanceof Error ? error.message : String(error));
    }
}
async function checkBackgroundProcesses(_client) {
    ...
    try {
        ...
        console.error('❌ Erreur lors de la vérification des processus:', error instanceof Error ? error.message : String(error));
    }
}
// Exécuter l'analyse
