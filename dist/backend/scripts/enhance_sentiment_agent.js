...
class EnhancedVortex500Agent extends Vortex500Agent {
     ...
     */
    determineMarketSession(now) {
        const hour = now.getHours();
        const day = now.getDay();
        if (day === 0 || day === 6)
            return 'weekend';
        if (hour < 9 || hour >= 16)
            return hour < 9 ? 'pre-market' : 'after-hours';
        return 'regular';
    }
    /**
     ...
     * Déterminer le régime de marché
     */
    determineMarketRegime(score, sentiment) {
     ...
     * Calculer la force du sentiment
     */
    calculateSentimentStrength(score) {
     ...
     */
    generateKeyInsights(baseData) {
        const insights = [];
        if (baseData.catalysts && baseData.catalysts.length > 0) {
            baseData.catalysts.forEach((catalyst) => {
     ...
     */
    generateTradingSignals(baseData, volatility) {
        const signals = {
            entry_signal: 'HOLD',
            exit_signal: 'NONE',
     ...
     * Évaluer l'impact des news
     */
    evaluateNewsImpact(newsCount, riskLevel) {
        if (newsCount > 20 && riskLevel === 'high')
            return 'critical';
        if (newsCount > 15)
            return 'high';
        if (newsCount > 10)
     ...
     * Créer les flags de validation
     */
    createValidationFlags(baseData) {
        return {
            data_valid: true,
     ...
     * Créer les métriques de performance
     */
    createPerformanceMetrics(baseData, durationMs) {
        return {
            inference_speed_ms: durationMs,
            data_points_analyzed: this.newsItemsData.length,
            processing_efficiency: durationMs < 5000 ? 'excellent' : durationMs < 10000 ? 'good' : 'acceptable',
    ...
    calculateDataQuality() {
        ...
        const sources = new Set(this.newsItemsData.map((item) => item.source));
    ...
    async analyzeMarketSentiment(forceRefresh = false) {
        ...
        try {
            ...
            const baseResult = await super.analyzeMarketSentiment(forceRefresh);
     ...
     */
    createNotAvailableEnhancedResult(durationMs) {
        return {
