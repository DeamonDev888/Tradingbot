...
async function databaseCleanup() {
    ...
    `;
        ...
        for (const indexSql of indexes) {
            try {
                ...
                console.log(`⚠️  Index déjà existant ou erreur: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    ...
    `;
        ...
        console.error('❌ Erreur lors du nettoyage:', error instanceof Error ? error.message : String(error));
    }
    finally {
        client.release();
        await pool.end();
