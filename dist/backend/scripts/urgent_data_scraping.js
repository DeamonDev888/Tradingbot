...
class UrgentDataScraping {
    ...
    async testConnections() {
        ...
        try {
            ...
            if (process.env.FINNHUB_API_KEY) {
                const testResponse = await fetch('https://finnhub.io/api/v1/news?category=general&token=' + process.env.FINNHUB_API_KEY);
    ...
    async executeUrgentScraping() {
        ...
        try {
            ...
            console.log(`   • Sources réussies: ${result.sourcesSuccessful.length}/${result.sourcesAttempted.length}`);
            ...
            if (result.success) {
                ...
                console.log(`   ✅ Objectif sources: ${result.sourcesSuccessful.length} >= ${targetSources}`);
            }
            else {
                ...
                if (result.sourcesSuccessful.length < targetSources) {
                    console.log(`   ❌ Sources insuffisantes: ${result.sourcesSuccessful.length} < ${targetSources}`);
                }
            }
        }
        catch (error) {
    ...
    async executeImmediateFollowUp() {
        ...
        try {
            ...
            const testNews = [
                {
                    ...
                    content: 'Les marchés financiers montrent une résilience surprenante face aux défis économiques actuels.',
                    timestamp: new Date(),
                    sentiment: 'bullish',
                },
                {
                ...
                },
                {
                    ...
                    content: 'Le secteur technologique fait face à des ventes massives amid tensions commerciales.',
                ...
                },
                {
                    ...
                    content: "Les prix du pétrole augmentent après que l'OPEC+ a annoncé des réductions de production.",
            ...
            for (const news of testNews) {
                try {
                    await this.dbService.saveNewsItems([news]);
                    insertedCount++;
                }
                catch (error) {
    ...
    formatReport(result) {
        ...
        lines.push(`   • Objectif (100+): ${result.itemsCollected >= 100 ? '✅ Atteint' : '❌ Manqué'}`);
        lines.push('');
        // Performance
        const itemsPerSecond = result.duration > 0 ? (result.itemsCollected / (result.duration / 1000)).toFixed(2) : '0';
        lines.push('⚡ PERFORMANCE:');
        lines.push(`   • Vitesse: ${itemsPerSecond} items/s`);
        lines.push(`   • Temps moyen: ${result.itemsCollected > 0 ? Math.round(result.duration / result.itemsCollected) : 0}ms/item`);
        lines.push('');
        // Erreurs
...
//# sourceMappingURL=urgent_data_scraping.js.map