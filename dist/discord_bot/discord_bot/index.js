...
const RougePulseAgentFixed_1 = require("../backend/agents/RougePulseAgentFixed");
...
client.on('messageCreate', async (message) => {
    ...
    if (message.content.trim().toLowerCase() === '!rougepulseagent') {
        ...
        const loadingMsg = await message.reply('🔴 **RougePulseAgentFixed** analyse le calendrier économique... ⏳');
        try {
            const agent = new RougePulseAgentFixed_1.RougePulseAgentFixed();
            ...
            if ('error' in result) {
                ...
                await loadingMsg.edit(`ℹ️ **RougePulseAgentFixed** : ${result.message}`);
            }
                ...
                await loadingMsg.edit('❌ **Erreur RougePulseAgentFixed** : Résultat invalide ou vide');
            }
        }
        catch (error) {
            console.error('Error in RougePulseAgentFixed command:', error);
            ...
            let userMessage = `❌ **Erreur RougePulseAgentFixed** : ${truncatedError}`;
            if (errorMessage.includes('Timeout')) {
                userMessage =
                    "⏰ **Timeout RougePulseAgentFixed** : L'analyse prend trop de temps. Réessayez plus tard.";
            }
            else if (errorMessage.includes('No significant events found')) {
                userMessage =
                    'ℹ️ **RougePulseAgentFixed** : Aucun événement économique significatif trouvé pour les prochaines 24h.';
            }
