...
import fs from 'fs';
import { loadServerInfo, fetchAndSave, refreshServerInfo } from './serverScanner'; // Import du module
dotenv.config();
...
client.once('ready', async () => {
    ...
    if (!serverInfo) {
        ...
        if (serverInfo)
            serverInfo.guild.name = guild.name;
    }
    ...
    if (channelId && serverInfo) {
        ...
        if (channel && channel.isTextBased()) {
            await channel.send(`Bot prêt ! ${serverInfo.channels.length} channels et ${serverInfo.members.length} membres connus.`);
        }
    }
});
